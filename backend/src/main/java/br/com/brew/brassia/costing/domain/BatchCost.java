...
public final class BatchCost {
    ...
    private BatchCost(UUID id, UUID breweryId, UUID batchId, String batchCode, BigDecimal volumeLiters,
            List<CostLine> lines, List<CostGap> gaps, boolean closed, UUID closedBy, Instant closedAt,
            String note) {
    ...
    public static BatchCost open(UUID breweryId, UUID batchId, String batchCode, BigDecimal volumeLiters,
            List<CostLine> lines, List<CostGap> gaps) {
        return new BatchCost(UUID.randomUUID(), breweryId, batchId, batchCode, volumeLiters, lines, gaps,
                false, null, null, null);
    }

    public static BatchCost reconstitute(UUID id, UUID breweryId, UUID batchId, String batchCode,
            BigDecimal volumeLiters, List<CostLine> lines, List<CostGap> gaps, UUID closedBy,
            Instant closedAt, String note) {
        return new BatchCost(id, breweryId, batchId, batchCode, volumeLiters, lines, gaps, true, closedBy,
                closedAt, note);
    }
    ...
    public BatchCost close(UUID actorId, String note, Instant at) {
        ...
        return new BatchCost(id, breweryId, batchId, batchCode, volumeLiters, lines, gaps, true,
    ...
    public BigDecimal total() {
        return lines.stream().map(CostLine::total).reduce(BigDecimal.ZERO, BigDecimal::add);
    }

    /** Total por categoria — é como o custo é lido: quanto foi malte, quanto foi lata. */
    public Map<CostCategory, BigDecimal> totalByCategory() {
        var totals = new EnumMap<CostCategory, BigDecimal>(CostCategory.class);
        for (CostLine line : lines) {
            totals.merge(line.category(), line.total(), BigDecimal::add);
        }
        return totals;
    }
     ...
     * precisa saber.
     */
    public BigDecimal costPerLiter() {
        if (volumeLiters.signum() <= 0) {
            return BigDecimal.ZERO;
        }
        return total().divide(volumeLiters, 4, RoundingMode.HALF_UP);
    }

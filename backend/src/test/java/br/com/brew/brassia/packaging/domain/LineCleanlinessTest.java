...
class LineCleanlinessTest {
    ...
    void blocksLineThatWasNeverCleaned() {
        var blocker = LineCleanliness.check(null, null, PLANNED_START);
    ...
    void acceptsCleanLineWithoutPreviousUse() {
        assertThat(LineCleanliness.check(YESTERDAY, null, PLANNED_START)).isEmpty();
    }
    ...
    void blocksCleaningReleasedAfterThePlannedStart() {
        ...
        assertThat(LineCleanliness.check(afterStart, null, PLANNED_START)).isPresent();
    }

    @Test
    void acceptsCleaningReleasedExactlyAtThePlannedStart() {
        assertThat(LineCleanliness.check(PLANNED_START, null, PLANNED_START)).isEmpty();
    }
    ...
    void blocksWhenTheLineWasUsedAgainAfterTheLastCleaning() {
        ...
        assertThat(LineCleanliness.check(LAST_WEEK, YESTERDAY, PLANNED_START)).isPresent();
        // Nem quando o uso é simultâneo à liberação.
        assertThat(LineCleanliness.check(YESTERDAY, YESTERDAY, PLANNED_START)).isPresent();
    }

    @Test
    void acceptsCleaningDoneAfterThePreviousPackagingRun() {
        assertThat(LineCleanliness.check(YESTERDAY, LAST_WEEK, PLANNED_START)).isEmpty();
    }
}

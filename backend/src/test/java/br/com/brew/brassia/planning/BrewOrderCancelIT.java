...
class BrewOrderCancelIT {
    ...
    @Test
    void cancelsReleasedOrder() throws Exception {
        var session = login();
        var orderId = draftOrder(session);
        mockMvc.perform(post("/api/v1/brew-orders/" + orderId + "/release").session(session).with(csrf())
                        .contentType("application/json").content("{\"assignedUserId\":\"" + UUID.randomUUID() + "\"}"))
                .andExpect(status().isOk());

        mockMvc.perform(post("/api/v1/brew-orders/" + orderId + "/cancel").session(session).with(csrf())
                        .contentType("application/json").content("{\"reason\":\"erro de programação\"}"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status", is("CANCELLED")));
    }
    ...
    void recancelIsIdempotent() throws Exception {
        var session = login();
        var orderId = draftOrder(session);
        var body = "{\"reason\":\"motivo\"}";
        mockMvc.perform(post("/api/v1/brew-orders/" + orderId + "/cancel").session(session).with(csrf())
                        .contentType("application/json").content(body))
                .andExpect(status().isOk());

        // Recancelar → 200 no-op (idempotente).
        mockMvc.perform(post("/api/v1/brew-orders/" + orderId + "/cancel").session(session).with(csrf())
                        .contentType("application/json").content(body))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status", is("CANCELLED")));
    ...
    void deniesWithoutPermissionAndAcrossBrewery() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/brew-orders/" + orderId + "/cancel")
    ...
    private String draftOrder(MockHttpSession session) throws Exception {
        ...
        var content = """
                {"name":"Cancel %s","equipmentId":"%s","batchVolumeLiters":400,"boilTimeMinutes":60,"targetIbu":30,

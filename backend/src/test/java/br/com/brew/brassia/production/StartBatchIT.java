...
class StartBatchIT {
    ...
    @Test
    void startingReleasedOrderCreatesBatchWithRoute() throws Exception {
        var session = login();
        var orderId = releasedOrder(session);

        mockMvc.perform(post("/api/v1/brew-orders/" + orderId + "/start").session(session).with(csrf()))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status", is("IN_PRODUCTION")));

        // O lote foi criado (evento síncrono) com roteiro derivado da receita.
        var body = mockMvc.perform(get("/api/v1/production/batches").session(session))
                .andExpect(status().isOk())
                .andReturn().getResponse().getContentAsString();
        var batch = batchByOrder(body, orderId);
        org.assertj.core.api.Assertions.assertThat(batch).isNotNull();

        mockMvc.perform(get("/api/v1/production/batches/" + batch).session(session))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status", is("IN_PROGRESS")))
                .andExpect(jsonPath("$.orderId", is(orderId)))
                .andExpect(jsonPath("$.steps.length()", greaterThanOrEqualTo(2)))
                .andExpect(jsonPath("$.steps[*].type", hasItem("MASH")))
                .andExpect(jsonPath("$.steps[*].type", hasItem("TRANSFER")));
    }

    @Test
    void restartingIsRejectedAndDoesNotDuplicateBatch() throws Exception {
        var session = login();
        var orderId = releasedOrder(session);
        mockMvc.perform(post("/api/v1/brew-orders/" + orderId + "/start").session(session).with(csrf()))
                .andExpect(status().isOk());
        // Transição única: reiniciar (já IN_PRODUCTION) → 409.
        mockMvc.perform(post("/api/v1/brew-orders/" + orderId + "/start").session(session).with(csrf()))
                .andExpect(status().isConflict());
    }

    @Test
    void cannotStartOrderNotReleased() throws Exception {
        var session = login();
        var orderId = draftOrder(session); // DRAFT
    ...
    @Test
    void deniesStartWithoutPermission() throws Exception {
        var session = login();
        var orderId = releasedOrder(session);

        mockMvc.perform(post("/api/v1/brew-orders/" + orderId + "/start")
                        .with(authentication(principal(UUID.randomUUID(), Set.of("planning.order.read")))).with(csrf()))
                .andExpect(status().isForbidden());
    }

    @Test
    void deniesBatchListingWithoutPermission() throws Exception {
        mockMvc.perform(get("/api/v1/production/batches")
                        .with(authentication(principal(UUID.randomUUID(), Set.of("planning.order.read")))))
                .andExpect(status().isForbidden());
    }
    ...
    private String draftOrder(MockHttpSession session) throws Exception {
        ...
        var content = """
                {"name":"Batch %s","equipmentId":"%s","batchVolumeLiters":400,"boilTimeMinutes":60,"targetIbu":30,

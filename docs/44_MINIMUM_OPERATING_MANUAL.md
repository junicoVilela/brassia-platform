...
| Custo e relatório do lote | Tela | `costing-journey.spec.ts`: o consumo da brassagem é confirmado, o total apurado é **a soma das parcelas**, o custo por litro cai sobre o volume transferido, a tela mostra esse total **com vírgula**, o dossiê mostra o **mesmo** número, a lacuna de mão de obra é declarada, o total **se move** quando a hora ganha preço, e fechar congela nas duas telas. Mais `BatchCostIT`, `BatchVarianceIT` e `BatchReportIT` pela API. |
...
| Bloqueadores | — | Não é teste: é o `STATUS.md` das sprints. Hoje, **um débito em aberto** — `DEB-TRC-003`: a tela do recall não mostra os contêineres do escopo; a regra existe e a leitura não chegou à interface. |
...
| **Offline** | Tela | `offline-sync-journey.spec.ts`: o reenvio devolve `DUPLICATE` **apontando para a prova que já existe**, a parada segue com uma só prova, e o conflito **aparece na tela do escritório** — com o número do aviso batendo com o da fila. Mais `SyncIT` pela API. |

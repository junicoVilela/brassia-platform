...
- SEC-F10: administração de federação (SAML/OIDC) entregue por completo (criar/listar/validar). ~~A parte SCIM não tem endpoint de sessão~~ — **admin de mapeamentos SCIM RESOLVIDO pelo SEC-B05** (#75): endpoints de sessão listar/upsert/desativar mapeamentos por provedor; a tela tem painel de mapeamentos (grupo externo → interno). ~~Vincular identidade externa não é listável~~ — **RESOLVIDO pelo SEC-B06** (#74). O provisionamento SCIM em si continua sendo executado pelo IdP via API key em `/scim/v2/**`. Débito remanescente: fluxo de login SSO no browser (SEC-B07, Sprint 15).

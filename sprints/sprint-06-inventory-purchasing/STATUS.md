...
- **Leitura pura**: não cria pedido de compra nem reserva. Permissão `purchasing.purchase.read` (V43).
- **DÉBITO PUR-001-A RESOLVIDO**: `reorderPoint` (estoque de segurança) por ingrediente e `leadTimeDays` no fornecedor (V48). A necessidade passou a ser `max(0, demanda + pontoDePedido − saldo)` e considera também ingredientes com ponto de pedido sem OP liberada; o lead time é informativo na lista de compras (badge por fornecedor). `reorderPoint` é convertido da unidade de uso para a canônica.
...
- **DÉBITO PUR-002-A RESOLVIDO**: `purchasePackageSize` no ingrediente do catálogo (V47, opcional). Quando presente e a conversão para a unidade de compra se aplica, a lista arredonda a quantidade para múltiplos de pacote fechado e informa `packages`; o custo estimado passa a refletir o comprado (pacote fechado). Sem tamanho de embalagem → comportamento anterior.
